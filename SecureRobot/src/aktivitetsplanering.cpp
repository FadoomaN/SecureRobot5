...
void setupVertexes() {
    ...
    // node30
    link(29, 30, 22.635f, 212.09f);

    // node31
    link(30, 31, 23.6376f, 215.94f);
    ...
    // node40
    link(39, 40, 0.72f, 315.f);

    // node41
    link(40, 41, 3.78f, 270.f);

    // node42
    link(41, 42, 28.5533f, 45.26f);
...
void startNetwork()
{
    ...
    xTaskCreatePinnedToCore(receiveFromServer, "receiveFromServer", 4096, NULL, 1, NULL, 1); //UDP mottagning
...
String plan() {
    ...
    for (int i = 0; i < robotList.size(); i++)
    {
        ...
        {
            workingRobots.push_back(&robotList[i]);
        }
    }
    //kollar vilka rum som saknar robotar och deligerar lediga robotar till dessa rum.
    for(int i = 0; i < nodeList.size(); i++)//temp
    {
        if(!nodeList[i].checked && !avelableRobots.empty() && !checkIfRoomInProgress(nodeList[i].room, workingRobots))
        {
            Serial.println("GAMMAL RUM: " + String(avelableRobots.back()->taskedRoom));
            avelableRobots.back()->taskedRoom = nodeList[i].room;
            Serial.println("NYTT RUM: " + String(avelableRobots.back()->taskedRoom));
...
bool allChecked(int targetRoom, const std::vector<Vertex>& nodeList) {
    for (const Vertex& v : nodeList) {
